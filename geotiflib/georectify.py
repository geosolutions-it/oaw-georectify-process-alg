...
class GeoRectifyFactory:
    ...
    def create(mode="OAW_TIF", *args, **kwargs):
        ...
        if mode == "OAW_TIF":
            ...
            return geo.pipe(
                AddGcpTask(input=in_tif, output=gcp_tif, min_points=min_points, log_location=output_folder)
            ).pipe(
                WarpTask(input=gcp_tif, output=grf_tif, log_location=output_folder)
            ).pipe(
                SeparateBandTask(input=grf_tif, output=bnd_vrt, log_location=output_folder)
            ).pipe(
                BinaryMaskTask(input=bnd_vrt, output=msk_tif, qgis_scripts=qgis_scripts, gdal_threads=gdal_threads, log_location=output_folder)
            ).pipe(
                RecombineBandMaskTask(input_vrt=bnd_vrt, input_mask=msk_tif, output=fin_vrt, log_location=output_folder)
            ).pipe(
                TileJpegTask(input=fin_vrt, output=fin_tif, log_location=output_folder)
            ).pipe(
                AddOverviewTask(input=fin_tif, log_location=output_folder)
            ).pipe(
                EditTagTask(input=in_tif, output=fin_tif, qgis_scripts=qgis_scripts, log_location=output_folder)
            ).pipe(
                CleanTask(input=[
                    gcp_tif, grf_tif,
